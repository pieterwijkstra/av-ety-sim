# AV ETY Simulation v8: Added Loops, introduced realistic parameters + VRS, vessel assignment from passive vessels, first created (=alphabetical)
import random
import pandas as pd
...
MEAN_PORT_DISRUPT_VALUES = [0, 0.25, 0.5]
    ...
    for no_cargoes in BFB_CARGOES:
        ...
        for no_vessels in NO_OF_VESSELS:
            ...
            for MEAN_PORT_DISRUPT in MEAN_PORT_DISRUPT_VALUES:
                ...
                log_filename = str(no_cargoes) + "cargoes-" + str(no_vessels) + "-vessels" + str(MEAN_PORT_DISRUPT)+"-avg_disrupt.txt"
                with open ('logs/'+log_filename, 'a') as log_file:
                    ...
                    with open ('logs/results.txt', 'a') as results_file:
                        results_file.write("Cargoes: {}, Vessels: {}, Mean Disruption: {}\n \n".format(no_cargoes, no_vessels, MEAN_PORT_DISRUPT))
